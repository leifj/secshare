/* AES code stolen from libgcrypt...
 * ported by Konrad Rosenbaum
 *
 * AES always has a block size of 16 bytes, the key length can be 128, 192, or
 * 256 bits (16, 24, 32 byte)
 */
...
typedef struct {
    ...
    int mode;
    byte cbcbuffer[16];/*used for CBC, CFB, etc.*/
} RIJNDAEL_context;
  ...
  if( keylen == 128/8 )
  ...
  else if ( keylen == 256/8 )
    {
      ...
      KC = 8;
          ...
          fprintf(stderr,"Internal error: invalid keylength %i.\n",keylen);
          return INV_KEYLEN;
  }
...
do_encrypt (const RIJNDAEL_context *ctx, byte *b, const byte *a)
{
  ...
  /* KR: what did Werner K. expect here? It's crypto code for crying out loud!*/
...
selftest (void)
{
  ...
  do_setkey (&ctx, key, sizeof(key));
  do_encrypt (&ctx, scratch, plaintext);
  ...
  do_setkey (&ctx, key_192, sizeof(key_192));
  do_encrypt (&ctx, scratch, plaintext_192);
  ...
  do_setkey (&ctx, key_256, sizeof(key_256));
  do_encrypt (&ctx, scratch, plaintext_256);
...
void* aes128_new(const void*key,int mode)
{
        ...
        ctx=malloc(sizeof(RIJNDAEL_context));
        if(ctx==NULL)return NULL;
        /*FIXME: add error checking*/
        do_setkey(ctx,key,16);
        ctx->mode=mode;
        if((mode&MASK_MODE)==CBC){
                int i;
                /*initialize CBC buffer, we assume that the IV is the first
                encrypted block, hence we init with zero*/
                for(i=0;i<16;i++)ctx->cbcbuffer[i]=0;
        }else{/*assume EBC*/
        }
        if((mode&MASK_CRYPT)==DECRYPT){
		prepare_decryption(ctx);
	}
        return ctx;
}

static void enc_block2(RIJNDAEL_context*ctx,byte*buf)
{
        if((ctx->mode&MASK_MODE)==CBC){
                int i;
                for(i=0;i<16;i++){
                        buf[i]^=ctx->cbcbuffer[i];
                }
                do_encrypt(ctx,buf,buf);
                for(i=0;i<16;i++){
                        ctx->cbcbuffer[i]=buf[i];
                }
        }else{/*EBC*/
                do_encrypt(ctx,buf,buf);
        }
...
int aes_encrypt2(void*ctx,byte*buf,int len)
{
        int outp=0;
        RIJNDAEL_context *c=ctx;

        while((outp+16)<=len){
                enc_block2(c,buf+outp);
                outp+=16;
        }

        return outp;
}
static void dec_block2(RIJNDAEL_context*ctx,byte*buf)
{
        if((ctx->mode&MASK_MODE)==CBC){
		byte cb[16];
                int i;
		memcpy(cb,buf,16);
                do_decrypt(ctx,buf,buf);
                for(i=0;i<16;i++){
                       buf[i]^=ctx->cbcbuffer[i];
                }
                memcpy(ctx->cbcbuffer,cb,16);
        }else{/*EBC*/
                do_decrypt(ctx,buf,buf);
        }
}

int aes_decrypt2(void*ctx,byte*buf,int len)
{
        int outp=0;
        RIJNDAEL_context *c=ctx;

        while((outp+16)<=len){
                dec_block2(c,buf+outp);
                outp+=16;
        }
        return outp;
}

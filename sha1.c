/* stolen from libgcrypt...
 * ported by Konrad Rosenbaum
 */
...
byte*sha1_get(void*ctx)
{
        sha1_final(ctx);
        return sha1_read(ctx);
}

void*sha1_new()
{
        SHA1_CONTEXT *c;
        c=malloc(sizeof(SHA1_CONTEXT));
        if(c==NULL)return NULL;
        sha1_init(c);
        return c;
}

void sha1_close(void*c)
{
        free(c);
}

char* sha1_prints(char*s,byte*h)
{
